...
- **Produccion accesible:** se ve mas avanzada que el repo local en autenticacion y en el formulario de alta, pero todavia se siente como una suma de superficies funcionales mas que como un sistema de producto cerrado.
- **Repo local:** conserva mucho ADN de starter Expo y decisiones visuales puntuales. Hay primitives, pero no hay todavia un sistema visual real.
- **Riesgo principal:** la divergencia entre lo desplegado y el codigo auditado impide tener una fuente de verdad visual. Antes de pulir detalle fino, hay que cerrar esa brecha.
...
### Evidencia derivada del repo local
...
Se ejecuto `npm run lint` tras instalar dependencias:

- Resultado: `0 errors`, `10 warnings`.
- Las warnings afectan a estado no usado, hook dependencies y variables de error sin usar.

### Limitaciones importantes

No se pudo completar un alta real de usuario con email desde la UI desplegada porque el acceso de produccion inspeccionado no expone registro por email en el primer flujo. La version local si contiene un `AuthScreen` con registro, pero su `.env.local` no carga `EXPO_PUBLIC_SUPABASE_URL` ni `EXPO_PUBLIC_SUPABASE_ANON_KEY`, por lo que no permite crear un usuario real localmente.

Por eso:

- Los estados autenticados de home con datos poblados se evaluan principalmente desde codigo local y arquitectura.
...
- El repo local mezcla pantallas de producto con restos de plantilla Expo (`explore`, `modal`).
- El home usa header de stack, settings desplegable, resumen, lista y CTA inferior sin una regla clara de layout por breakpoint.
- El formulario local usa secciones repetidas por padding, pero no una gramatica clara de section card, field group, action footer o supporting text.
- El despliegue visto en produccion ya introduce section cards y chips mas elaborados en el alta, pero esa linea visual no aparece alineada con las primitives del repo local.
...
- En el repo local el gate de auth vive dentro de `HomeScreen`, no en un limite de navegacion mas explicito. Eso deja mas facil que rutas secundarias tengan reglas distintas.
...
- `app/(tabs)/explore.tsx` y `app/modal.tsx` son deuda visible de starter. No deberian existir en una superficie de producto final.

### 3.3 Jerarquia visual

La jerarquia actual es inconsistente:

- El home local trata el header del navegador y el CTA inferior como principales, pero no hay una capa intermedia fuerte para estado de la cuenta, proximas renovaciones o prioridad temporal.
- Los summary cards locales compiten por color con acciones y estados; solo usan azul y no comunican prioridad.
...
- El formulario desplegado de produccion secciona mucho; eso mejora scanning, pero tambien crea una experiencia larga y pesada si demasiadas opciones avanzadas aparecen antes de que el usuario haya guardado la renovacion minima.
...
- los estilos recuerdan demasiado a iOS defaults o starter Expo;
...
### H1. Produccion y repo local no comparten una fuente visual de verdad

**Observado + codigo.** Produccion muestra auth con Apple/Google y un formulario de alta mas rico; el repo local muestra auth email login/registro y un formulario local mas reducido. Esta brecha complica cualquier auditoria, QA y refactor visual.

**Impacto:** alto en mantenibilidad, consistencia y velocidad de mejora.

### H2. Auth y limites de acceso son inconsistentes

**Observado + codigo.** El home local decide auth dentro de la pantalla. Produccion deja ver `/renewal/new` sin haber pasado por acceso. El usuario no recibe una politica clara: que puede explorar, que requiere cuenta y cuando se le pedira sesion.
...
### H6. Rutas y contenido de starter siguen presentes

**Codigo.** `explore` y `modal` contienen texto de plantilla Expo. Aunque parte no se despliegue, esa deuda es una señal fuerte de producto inacabado.
...
La validacion local de auth y renewal usa `Alert.alert`. En web eso se siente pobre, interrumpe contexto y no deja errores pegados al campo.

### M3. Estados de carga, error y exito son pobres

Home tiene spinner central; settings tiene spinner en card; `useRenewals` guarda `error` pero home no lo renderiza. Falta diferencia entre loading inicial, refresh, save pending, empty filtered, network error y retry.
...
En el repo local `IconSymbol` mapea muy pocos iconos Material para web mientras el dominio usa muchos nombres de SF Symbols. Algunos usos se fuerzan con `as any`.

### M6. El resumen financiero es incompleto

Home solo muestra mensual y anual. No diferencia renovaciones proximas, vencidas, coste convertido por frecuencia, moneda mixta ni tendencia temporal. Visualmente parece widget, no decision support.
...
- En el despliegue inspeccionado no se ve registro por email ni aclaracion de como crear cuenta.
- La pantalla se parece a un auth shell generico. No vende ni prepara el trabajo del usuario.
- En local el formulario email esta en una card blanca con layout iOS-like y footer toggle; en produccion el lenguaje visual ya es otro.
...
- El home local no diferencia informacion operativa "que vence pronto" de resumen financiero.
- Settings aparece como expansion dentro del mismo scroll; eso mezcla dashboard y configuracion.
- Header actions son demasiado crudas: un emoji de engranaje y `Salir` destructivo compiten con la tarea principal.
- El CTA inferior es util en movil, pero debe convivir con teclado, tab bar, safe area y scrolling largo.
...
- En local validation va por alerts; en produccion hay riesgo de formulario demasiado largo.
...
- El componente local cubre metodo, toggle y datos requeridos.
...
- Empty state local no incluye CTA integrado aunque CTA inferior exista.
...
- Targets minimos de `Button` local son razonables (`minHeight: 50`), pero chips/options no tienen una regla de minimo tactil.
...
- Icon-only action `⚙️` en local no modela label accesible.
...
- Auth local y produccion no comparten tono.
...
1. Eliminar o esconder rutas de starter (`explore`, `modal`) que no sean producto.
...
10. Reducir primera version del formulario a campos esenciales y colapsar advanced si la version desplegada es la nueva fuente de verdad.
...
### Fase 0. Cerrar fuente de verdad

- Elegir si produccion actual o branch local sera base.
- Alinear auth, rutas y formulario.
...
RenovacionesApp tiene buen scope para volverse muy agradable: el problema a resolver es concreto y el usuario vuelve por fechas, dinero y tranquilidad. Para que se sienta premium, el siguiente paso no es pintar botones al azar. Es **unificar fuente de verdad, formalizar primitives y rediseñar los dos flujos que mas pesan en movil: home/listado y alta/edicion**.
